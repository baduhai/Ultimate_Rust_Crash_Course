...
fn main() {
    ...
    for arrow in arrow_coords {
        ...
        } else if 1.0 <= arrow.distance_from_center() && arrow.distance_from_center() <= 5.0 {
            shots.push(Shot::Hit(arrow.distance_from_center()));
        } else {
            shots.push(Shot::Miss);
        }
        // Also solveable with `match` (and much cleaner):
